...
class CorrectiveRAG:
    ...
    def evaluate_context(
        ...
        # If cross-encoder score is clearly high, fast-path accept
        if highest_score >= 1.0:
            logger.info(f"CRAG: High cross-encoder score ({highest_score:.3f}). Context accepted as RELEVANT.")
            return "RELEVANT", False

        # If score is questionable, verify with LLM evaluator
